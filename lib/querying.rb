...
def select_value_and_count_of_most_prolific_species
  # "select id, species from characters order by count(species) limit 1"
...
def select_character_names_and_number_of_books_they_are_in
  "select characters.name, count(*) from character_books join characters on character_books.character_id = characters.id group by characters.name ORDER BY count(characters.name) DESC"
end
